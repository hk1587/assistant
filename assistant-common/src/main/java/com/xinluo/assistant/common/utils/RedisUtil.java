...
public class RedisUtil {
    ...
    public void kickOutPotentialUser(String userOperation,String userPre,String mobileNum){
        //用户token前缀
        String userTokenKey = userPre + mobileNum;

        //从redis获取, 如果存在则删除
        Object token = get(userTokenKey);
     ...
     * @desc : 校验用户token信息
     * @param : obj 传入的对象
     * @param : responseDTO 校验信息
     */
    public ResponseDTO checkToken(Object obj, ResponseDTO responseDTO) throws Exception {
        ...
        if(encodeToken == null || CommonConstant.EMPTY_STRING.equals(encodeToken.trim())){
            responseDTO.setStateCode(StateConstant.AUTHENTICATION_FAILED);
            responseDTO.setMessage(CommonConstant.RESPONSE_MESSAGE_FAIL);
            responseDTO.setMessageDetail("用户身份认证不存在");
            return responseDTO;
        }
        //解密 token
        String token = RSAEncryption.RSADecode(RSAEncryption.getPrivateKey(privateKey), encodeToken);
        if(token == null){
            responseDTO.setStateCode(StateConstant.AUTHENTICATION_FAILED);
            responseDTO.setMessage(CommonConstant.RESPONSE_MESSAGE_FAIL);
            responseDTO.setMessageDetail("用户身份认证不正确");
            return responseDTO;
        } else {
            Object userObj = get(token);
            if ( userObj == null ) {
                responseDTO.setStateCode(StateConstant.AUTHENTICATION_FAILED);
                responseDTO.setMessage(CommonConstant.RESPONSE_MESSAGE_FAIL);
                responseDTO.setMessageDetail("账户身份认证过期");
                return responseDTO;
            } else {
                if (token.startsWith(CommonConstant.TOKEN_USER_PRE)){
                    Field userIdIdField = obj.getClass().getDeclaredField("userId");
                    userIdIdField.setAccessible(true);
                    User user = (User) (get(token));
                    userIdIdField.set(obj,user.getUserId());
                } 
            }
        }
        return responseDTO;
    ...
    public String  getToken(ResponseDTO responseDTO) throws Exception {
        HttpServletRequest request = ((ServletRequestAttributes) RequestContextHolder.getRequestAttributes()).getRequest();
